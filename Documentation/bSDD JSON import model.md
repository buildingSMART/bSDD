...
| <span id="IfcDataType">IfcDataType</span>                   | Text         |  ✅       |             | The IFC data type the property is expressed in. Can be more specific than [DataType](#DataType), for example `IfcLabel`, `IfcText`, `IfcIdentifier`, `IfcBoolean`, `IfcLogical`, `IfcInteger` or `IfcReal`. Any valid IFC defined type name can be used. |
