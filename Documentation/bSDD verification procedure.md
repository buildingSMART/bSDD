...
| Code   | Item                                                         |
| ------ | ------------------------------------------------------------ |
| GEN-01 | English version should be provided for all relevant fields   |
| GEN-02 | Translations should be accurate without changes              |
| GEN-03 | Names should be clear and easy to interpret                  |
| GEN-04 | Names and codes should follow a consistent naming convention |
| D-01   | Dictionary should in active status                           |
| D-02   | Dictionary name should not be misleading or suggest they represent something else |
| D-03   | Dictionary procedure information should be complete and transparent |
| P-01   | Numeric property should have Unit, Dimension, and correct data type |
| P-02   | Duplicated IFC properties should be avoided                  |
| P-03   | Using PSET_ in property or property set names is not allowed |
| P-04   | Data type of property should match the definition            |
| P-05   | Avoid unnecessary allowed values when simple data types can represent the value |
| C-01   | Classes should be mapped to IFC correctly                    |
| C-02   | Avoid circular dependencies in ParentClassCode               |
| CP-01  | External properties should not be used in ClassProperty      |
| CP-02  | ClassProperty should have PropertySet named                  |
| CR-01  | Avoid circular relationships in RelationType                 |
| CR-02  | Avoid incorrect class types in RelationType                  |
| PR-01  | Avoid unclear circular relationships in RelationType         |
| SEM-01 | Correct types should be assigned                             |
| SEM-02 | Dimension-Unit match must be accurate                        |
| SEM-03 | Classes with multiple levels should be structured hierarchically |
| SEM-04 | Relations must be meaningful                                 |
| SEM-05 | Avoid creating syncretic classes that combine multiple aspects |
| SEM-06 | Avoid mixing multiple aspects within a single property       |
| SEM-07 | AllowedValues must be distinct and represent alternative options |

**Verification item code naming convention: GEN (General), D (Dictionary), C (Class), P (Property), A (AllowedValue), CP (ClassProperty), CR (ClassRelation), PR (PropertyRelation), SEM (Semantics) with two digit number using dash separator.*
...
- The property name using acronym `FR-MR` could stand for "Fire Rating Minimal Requirement" or "Mechanical Resistance with country code FR," leading to confusion.
...
- The names `CLS03 Load Capacity` (title case), `CLS04 Power zone` (sentence case), and `CLP-01 ZoneCategories` (pascal case, code with a dash separator) would be clearer if the same convention were used across the dictionary.
...
The dictionary's name must clearly and accurately describe its content and purpose. It should not be misleading or suggest any association with other dictionaries or organizations. Please do not include other dictionaries name or organization's name.
